<script type="text/template" id="fusion-builder-block-module-image-compare-preview-template">
	<h4 class="fusion_module_title"><span class="fusion-module-icon {{ fusionAllElements[element_type].icon }}"></span>{{ fusionAllElements[element_type].name }}</h4>
	<div>
		<#
		var
		content = typeof params.element_content !== 'undefined' ? params.element_content : '',
		slider_reg_exp = window.wp.shortcode.regexp( 'fusion_compare_image' ),
		slider_inner_reg_exp = new RegExp( '\\[(\\[?)(fusion_compare_image)(?![\\w-])([^\\]\\/]*(?:\\/(?!\\])[^\\]\\/]*)*?)(?:(\\/)\\]|\\](?:([^\\[]*(?:\\[(?!\\/\\2\\])[^\\[]*)*)(\\[\\/\\2\\]))?)(\\]?)' ),
